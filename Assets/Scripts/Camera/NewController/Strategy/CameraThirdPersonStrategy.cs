...
public class CameraThirdPersonStrategy : ICameraStrategy
{
    ...
    public void OnLateUpdate()
    {
        ...
        // _camTransform.position = _squirrelThirdPersonLookObj.position + (Vector3.one * 0.1f) + _rotation * _offsetThirdPersonPosition; comente esto
        _camTransform.position = Vector3.Lerp(_camTransform.position, _squirrelThirdPersonLookObj.position + (Vector3.one * 0.1f) + _rotation * _offsetThirdPersonPosition, 0.1f);
        // iTween.MoveTo(_camTransform.gameObject, _squirrelThirdPersonLookObj.position + (Vector3.one * 0.1f) + _rotation * _offsetThirdPersonPosition, 0.1f); 

        _currentX = _currentX % 360;
        _camTransform.LookAt(_squirrelThirdPersonLookObj.position);
        //iTween.LookTo(_camTransform.gameObject, _squirrelThirdPersonLookObj.position, 0.1f);
